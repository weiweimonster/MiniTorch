...
def tensor_reduce(
    ...
    def _reduce(
        ...
        if out_pos < out_size:
            ...
            if pos == 0:
                out[o] = cache[0]
        # idx = out_index[reduce_dim] * cuda.blockDim.x + pos
        # a_reduce = a_shape[reduce_dim]
        # to_index(out_pos, out_shape, out_index)
        #
        # if idx < a_reduce:
        ...
        #     cuda.syncthreads()
        #
        # if idx < a_reduce and pos == 0:
        #     count = reduce_value
        #     for i in range(a_reduce):
        #         count = fn(count, cache[i])
        #     out[out_pos] = count
...
def _mm_practice(out: Storage, a: Storage, b: Storage, size: int) -> None:
    ...
    * Only write to global memory once per kernel.
    ...
    #     shared_memory_a[cuda.threadIdx.x, cuda.threadIdx.y] = a[col + row * size]
    #     shared_memory_b[cuda.threadIdx.x, cuda.threadIdx.y] = b[col + row * size]
    #     cuda.syncthreads()
    #
    #     total = 0.0
    #     for k in range(BLOCK_DIM):
    #         if k < size:
    #             total += shared_memory_a[cuda.threadIdx.x, k] * shared_memory_b[k, cuda.threadIdx.y]
    #
    ...
    * Only write to global memory once per kernel.
    ...
    for k_start in range(0, a_shape[2], BLOCK_DIM):
        ...
        if i < a_shape[1] and k < a_shape[2]:
            a_shared[pi, pj] = a_storage[a_batch_stride * batch + a_strides[1] + a_strides[2] * k]
        k = k_start + pi
        if i < b_shape[2] and k < b_shape[1]:
    ...
    # r_out, col_out = out_shape[1], out_shape[2]
    # out_rstride, out_col_stride = out_strides[1], out_strides[2]
    # a_row, a_col = a_shape[1], a_shape[2]
    # a_rstride, a_col_stride = a_strides[1], a_strides[2]
    # b_row, b_col = b_shape[1], b_shape[2]
    # b_rstride, b_col_stride = b_strides[1], b_strides[2]
    # count = 0.0
    #
    # for i in range(0, a_col, BLOCK_DIM):
    #     k = i + pj
    #     if k < a_col and i < a_row:
    #         a_idx = batch * a_batch_stride + i * a_rstride + k * a_col_stride
    #         a_shared[pi, pj] = a_storage[a_idx]
    #     k = i + pi
    #     if k < b_row and j < b_col:
    #         b_idx = batch * b_batch_stride + k * b_rstride + j * b_col_stride
    #         b_shared[pi, pj] = b_storage[b_idx]
    #     cuda.syncthreads()
    #     for k in range(BLOCK_DIM):
    #         if (i + k) < a_col:
    #             count = a_shared[pi, k] * b_shared[k, pj] + count
    # if i < r_out and j < col_out:
    #     out_idx = (out_strides[0] * batch + out_rstride * i + out_col_stride * j)
    #     out[out_idx] = count
